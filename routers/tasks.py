...
@router.get("/task/get_all",response_model=list[TaskResponse])
...
def add_to_db(task:TaskModels,user:User=Depends(get_current_user),db: Session=Depends(get_db)):
    return add_to_db_server(task,user.id,db)

@router.put("/task/put")
def update(id:int,task:TaskModels,user:User=Depends(get_current_user),db: Session=Depends(get_db)):
    return update_server(id,task,user.id,db)

@router.delete("/task/delete")
def delete_from_db(id: int,user:User=Depends(get_current_user),db: Session=Depends(get_db)):
